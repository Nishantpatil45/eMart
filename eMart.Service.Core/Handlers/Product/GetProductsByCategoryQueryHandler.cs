...
{
    public class GetProductsByCategoryQueryHandler : IRequestHandler<GetProductsByCategoryQuery, CommonResponse<List<ProductCommonResponseDto>>>
    {
        private readonly IProductRepository _productRepository;
        private readonly IUserRepository _userRepository;

        public GetProductsByCategoryQueryHandler(IProductRepository productRepository, IUserRepository userRepository)
        {
            _productRepository = productRepository;
            _userRepository = userRepository;
        }

        public async Task<CommonResponse<List<ProductCommonResponseDto>>> Handle(GetProductsByCategoryQuery request, CancellationToken cancellationToken)
        {
            try
            {
                // Get user details
                ...
                var products = await _productRepository.GetProductsByCategoryId(request.CategoryId, user);
