...
{
    public class GetOrdersByBuyerQueryHandler : IRequestHandler<GetOrdersByBuyerQuery, CommonResponse<List<OrderCommonResponseDto>>>
    {
        private readonly IOrderRepository _orderRepository;

        public GetOrdersByBuyerQueryHandler(IOrderRepository orderRepository)
        {
            _orderRepository = orderRepository;
        }

        public async Task<CommonResponse<List<OrderCommonResponseDto>>> Handle(GetOrdersByBuyerQuery request, CancellationToken cancellationToken)
        {
            try
            {
                var orders = await _orderRepository.GetByBuyerIdAsync(request.BuyerId, request.Page, request.PageSize);
